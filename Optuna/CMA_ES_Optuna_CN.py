...
def run(
    ...
    print("---------------Running Optuna for ChiN parameter learning---------------")
    ...
    np.savez(
        f"Optuna/opt_cn_params_{dimension}D_{instance}I_{p_class}C.npz",
        **best_params,
    )
    ...
    g_utils.save_results(
        results=results, policy=f"optuna_cn_{dimension}D_{instance}I_{p_class}C"
    )
