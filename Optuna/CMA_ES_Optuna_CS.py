...
def run(
    ...
    print("---------------Running Optuna for decay rate (cs) parameter learning---------------")
    ...
    np.savez(
        f"Optuna/opt_cs_params_{dimension}D_{instance}I_{p_class}C.npz",
        **best_params,
    )
    ...
    g_utils.save_results(
        results=results, policy=f"optuna_cs_{dimension}D_{instance}I_{p_class}C"
    )
