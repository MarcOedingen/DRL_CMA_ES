...
def run(
    ...
    print("---------------Running Optuna for damping (dp) parameter learning---------------")
    ...
    np.savez(
        f"Optuna/opt_dp_params_{dimension}D_{instance}I_{p_class}C.npz",
        **best_params,
    )
    ...
    g_utils.save_results(
        results=results, policy=f"optuna_dp_{dimension}D_{instance}I_{p_class}C"
    )
