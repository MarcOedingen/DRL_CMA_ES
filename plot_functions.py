...
func_ids = [16, 21]  # IDs for Weierstrass and Gallagher's Gaussian 101-me Peaks, as examples
dimension = 3
instances = [1, 5, 10]
domain = [-5, 5]
n_points = int(1e3)
bbob_functions = [BenchmarkFunction("bbob", fid, dimension, i) for fid in func_ids for i in instances]
...
for func in bbob_functions:
    ...
    for j in range(n_points):
        for k in range(n_points):
            Z[j, k] = func([X[j, k], Y[j, k], 0])  # Assuming a 3D function with a fixed 3rd parameter
    Zs.append(Z)
