...
def run(
    ...
    print(
        "---------------Running iterative imitation learning for variable parameters adaptation---------------"
    )
    ...
    with open(
            f"Environments/Combined/Policies/ppo_policy_comb_imit_iter_{dimension}D_{instance}I_{p_class}C.pkl",
            "wb",
    ) as f:
    ...
    g_utils.save_results(
        results=results,
        policy=f"ppo_policy_comb_imit_iter_{dimension}D_{instance}I_{p_class}C",
    )
