...
def run(
    ...
    print("---------------Running learning for static parameter adaptation---------------")
    ...
    ppo_model = g_utils.train_load_model(
        policy_path="Environments/Combined/Policies/ppo_policy_st",
        dimension=dimension,
        instance=instance,
        split=split,
        p_class=p_class,
    ...
    g_utils.save_results(
        results=results, policy=f"ppo_policy_st_{dimension}D_{instance}I_{p_class}C"
    )
