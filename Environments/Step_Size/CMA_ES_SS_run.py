...
def main():
    ...
    train_funcs = np.repeat([BenchmarkFunction("bbob", train_id, dim, 1) for train_id in train_ids], 20)
    np.random.shuffle(train_funcs)
    test_funcs = [BenchmarkFunction("bbob", test_id, dim, 1) for test_id in test_ids]
    test_funcs = np.repeat(test_funcs, 10)

    train_env = CMA_ES_SS(objetive_funcs=train_funcs, sigma=0.5)
    ...
    for index, test_func in enumerate(test_funcs):
        eval_env = CMA_ES_SS(objetive_funcs=[test_func], sigma=0.5)
