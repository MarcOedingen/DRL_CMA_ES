...
def run(dimension, x_start, sigma, instance):
    ...
    if os.path.exists(f"Environments/Step_Size/Policies/ppo_policy_ss_{dimension}D_{instance}I.pkl"):
        ppo_model.policy = pickle.load(open(f"Environments/Step_Size/Policies/ppo_policy_ss_{dimension}D_{instance}I.pkl", 'rb'))
    else:
        ppo_model.learn(
            total_timesteps=int(1e6), callback=ss_utils.StopOnAllFunctionsEvaluated()
        )
        pickle.dump(ppo_model.policy, open(f"Environments/Step_Size/Policies/ppo_policy_ss_{dimension}D_{instance}I.pkl", 'wb'))
