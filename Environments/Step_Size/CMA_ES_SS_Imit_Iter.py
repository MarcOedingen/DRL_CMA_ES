...
def run(
    ...
    for i in range(iterations):
        ...
        transition_splits.append(transitions[int(trans_split_indices[curr_index]): trans_split_indices[index_split]])
    ...
    with open(f"Environments/Step_Size/Policies/ppo_policy_ss_imit_iter{dimension}D_{instance}I_{p_class}C.pkl", "wb") as f:
    ...
    g_utils.save_results(
        results=results, policy=f"ppo_policy_ss_imit_iter{dimension}D_{instance}I_{p_class}C"
    )
