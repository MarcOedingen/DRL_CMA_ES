...
def create_Transitions(data):
    ...
    condition = np.all(data['observations'] == np.concatenate([np.array([0.5]), np.zeros(81)]), axis=1)
...
def run(dimension, x_start, sigma, instance):
    ...
    train_funcs = np.repeat([BenchmarkFunction("bbob", int(train_id), dimension, instance) for train_id in train_ids],
                            100)
    ...
    expert_samples = collect_expert_samples(dimension=dimension, x_start=x_start, sigma=sigma,
