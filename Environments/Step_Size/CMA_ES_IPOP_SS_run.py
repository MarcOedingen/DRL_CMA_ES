...
from Environments.Step_Size.CMA_ES_IPOP_SS_Env import CMA_ES_IPOP_SS
...
def run(
    ...
    print("---------------Running learning for step-size (ipop) adaptation---------------")
    ...
    ppo_model = g_utils.train_load_model(
        policy_path="Environments/Step_Size/Policies/ppo_policy_ipop_ss",
        dimension=dimension,
        instance=instance,
        split=split,
        p_class=p_class,
    ...
    g_utils.save_results(
        results=results, policy=f"ppo_policy_ipop_ss_{dimension}D_{instance}I_{p_class}C"
    )
