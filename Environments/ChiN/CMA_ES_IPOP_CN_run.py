...
def run(
    ...
    print("---------------Running learning for ChiN (ipop) adaptation---------------")
    ...
    ppo_model = g_utils.train_load_model(
        policy_path="Environments/ChiN/Policies/ppo_policy_ipop_cn",
        dimension=dimension,
        instance=instance,
        split=split,
        p_class=p_class,
    ...
    g_utils.save_results(
        results=results, policy=f"ppo_policy_ipop_cn{dimension}D_{instance}I_{p_class}C"
    )
