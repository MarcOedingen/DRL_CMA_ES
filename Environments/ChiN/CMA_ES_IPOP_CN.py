...
def run_CMAES_IPOP_CN(objective_fct, x_start, sigma, h=40, f_limit=np.power(10, 28)):
...
def collect_expert_samples(
    ...
    if os.path.isfile(
        f"Environments/ChiN/Samples/CMA_ES_IPOP_CN_Samples_{dimension}D_{instance}I_{p_class}C.npz"
    ):
        data = np.load(
            f"Environments/ChiN/Samples/CMA_ES_IPOP_CN_Samples_{dimension}D_{instance}I_{p_class}C.npz"
        )
    ...
    for function in tqdm(bbob_functions):
        ...
        obs, act, done = run_CMAES_IPOP_CN(
...
class CMAES_IPOP_CN:
    def __init__(self, x_start, sigma, parameters={}):
        ...
        if parameters:
            self.params.set_params(parameters)
