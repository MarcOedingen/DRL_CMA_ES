...
def run(
    ...
    with open(
        f"Environments/Mu_Effective/Policies/ppo_policy_me_imit_iter_{dimension}D_{instance}I_{p_class}C.pkl",
        "wb",
    ) as f:
    ...
    g_utils.save_results(
        results=results,
        policy=f"ppo_policy_me_imit_iter_{dimension}D_{instance}I_{p_class}C",
    )
