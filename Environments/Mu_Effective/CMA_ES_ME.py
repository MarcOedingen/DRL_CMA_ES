...
def run_CMAES_ME(objective_fct, x_start, sigma, h=40, f_limit=np.power(10, 28)):
    es = CMAES_ME(x_start, sigma)
    start_state = np.array([es.params.mueff, np.power(np.sum(es.params.weights), 2), np.sum(np.power(es.params.weights, 2))])
    ...
    while not es.stop():
        ...
        actions.append(es.params.mueff)
        dones.append(False)
        iteration += 1
    dones[-1] = True
...
def collect_expert_samples(dimension, instance, x_start, sigma, bbob_functions):
    if os.path.isfile(
        f"Environments/Mu_Effective/Samples/CMA_ES_ME_Samples_{dimension}D_{instance}I.npz"
    ):
        data = np.load(
            f"Environments/Mu_Effective/Samples/CMA_ES_ME_Samples_{dimension}D_{instance}I.npz"
        )
