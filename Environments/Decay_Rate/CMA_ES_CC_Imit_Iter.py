...
def run(
    ...
    print(
        "---------------Running iterative imitation learning for decay-rate (cc) adaptation---------------"
    )
    ...
    with open(
        f"Environments/Decay_Rate/Policies/ppo_policy_cc_imit_iter_{dimension}D_{instance}I_{p_class}C.pkl",
        "wb",
    ) as f:
    ...
    g_utils.save_results(
        results=results,
        policy=f"ppo_policy_cc_imit_iter_{dimension}D_{instance}I_{p_class}C",
    )
