...
def run_CMAES_PC(objective_fct, x_start, sigma, h=40, f_limit=4.6*np.power(10, 18)):
    es = CMAES_PC(x_start, sigma)
    start_state = np.array([es.params.cc, np.sqrt(es.params.cc * (2 - es.params.cc) * es.params.mueff), sigma, objective_fct.dimension])
    ...
    while not es.stop():
        ...
        pc, h_sig, x_old, arx = es.tell1(X, fit)
        es.pc = pc
        sigma = es.tell2(arx=arx, x_old=x_old, h_sig=h_sig)
        f_best = np.min(fit)
        if iteration > 0:
            difference = np.clip(
        ...
        observations.append(
            np.concatenate(
                [
                    ...
                    np.array([np.sqrt(es.params.cc * (2 - es.params.cc) * es.params.mueff)]),
                    np.array([sigma]),
                    np.array([objective_fct.dimension]),
                    np.pad(pc, (0, pad_size), "constant") if pad_size > 0 else pc,
                    np.array(hist_fit_vals)
                ]
            )
        )
        actions.append(np.pad(pc, (0, pad_size), "constant") if pad_size > 0 else pc)
        dones.append(False)
        iteration += 1
    dones[-1] = True
...
def collect_expert_samples(
    ...
    if os.path.isfile(
        f"Environments/Evolution_Path/Samples/CMA_ES_PC_Samples_{dimension}D_{instance}I_{p_class}C.npz"
    ):
        data = np.load(
            f"Environments/Evolution_Path/Samples/CMA_ES_PC_Samples_{dimension}D_{instance}I_{p_class}C.npz"
        )
...
class CMAES_PC:
    ...
    def tell1(self, arx, fit_vals):
        ...
        expert_pc = (1 - self.params.cc) * self.pc + h_sig * np.sqrt(
            self.params.cc * (2 - self.params.cc) * self.params.mueff
        ) * (self.x_mean - x_old) / self.sigma

        return expert_pc, h_sig, x_old, arx

    def tell2(self, arx, x_old, h_sig):
