...
class CMA_ES_PS(gymnasium.Env):
    ...
    def step(self, action):
        new_ps = action[: self.objective_funcs[self.curr_index].dimension]

        # Run one iteration of CMA-ES
        X = self.cma_es.ask()
        fit = [self.objective_funcs[self.curr_index](x) for x in X]
        _, x_old, arx = self.cma_es.tell1(X, fit)

        # Update variables
        self.cma_es.ps = new_ps

        sigma = self.cma_es.tell2(
            arx=arx, x_old=x_old, N=self.objective_funcs[self.curr_index].dimension
        )
