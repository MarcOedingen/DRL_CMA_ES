...
def run(
    ...
    print(
        "---------------Running imitation learning for evolution path (pc) adaption---------------"
    )
    ...
    policy_path = "Environments/Evolution_Path/Policies/policy_pc_imit"
    ...
    g_utils.save_results(
        results=results,
        policy=f"ppo_policy_pc_imit_{dimension}D_{instance}I_{p_class}C",
    )