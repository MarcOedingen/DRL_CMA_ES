...
from Environments.Evolution_Path.CMA_ES_PS import collect_expert_samples
...
def run(
    ...
    print(
        "---------------Running iterative imitation learning for evolution path (ps) adaption---------------"
    )
    ...
    with open(
            f"Environments/Evolution_Path/Policies/ppo_policy_ps_imit_iter_{dimension}D_{instance}I_{p_class}C.pkl",
            "wb",
    ) as f:
    ...
    g_utils.save_results(
        results=results,
        policy=f"ppo_policy_ps_imit_iter_{dimension}D_{instance}I_{p_class}C",
    )