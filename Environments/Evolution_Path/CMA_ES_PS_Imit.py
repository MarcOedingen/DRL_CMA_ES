...
def run(
    ...
    print(
        "---------------Running imitation learning for evolution path (ps) adaption---------------"
    )
    ...
    policy_path = "Environments/Evolution_Path/Policies/policy_ps_imit"
    ...
    g_utils.save_results(
        results=results,
        policy=f"ppo_policy_ps_imit_{dimension}D_{instance}I_{p_class}C",
    )