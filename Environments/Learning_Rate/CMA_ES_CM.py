...
def run_CMAES_CM(objective_fct, x_start, sigma, h=40, f_limit=4.6 * np.power(10, 18)):
    es = CMAES_CM(x_start, sigma)
    start_state = np.array([es.params.cmu, objective_fct.dimension])
    observations, actions, dones = [np.hstack((start_state, np.zeros(80)))], [], []
    hist_fit_vals = deque(np.zeros(h), maxlen=h)
    hist_cm = deque(np.zeros(h), maxlen=h)
    iteration = 0
    while not es.stop():
        X = es.ask()
...
def collect_expert_samples(
    ...
    if os.path.isfile(
        f"Environments/Learning_Rate/Samples/CMA_ES_CM_Samples_{dimension}D_{instance}I_{p_class}C.npz"
    ):
        data = np.load(
            f"Environments/Learning_Rate/Samples/CMA_ES_CM_Samples_{dimension}D_{instance}I_{p_class}C.npz"
        )
