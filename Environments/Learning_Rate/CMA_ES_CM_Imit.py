...
from Environments.Learning_Rate.CMA_ES_CM import collect_expert_samples
...
def run(
    ...
    print(
        "---------------Running imitation learning for learning-rate (cmu) adaptation---------------"
    )
    ...
    train_env = TimeLimit(
        CMA_ES_CM(objective_funcs=train_funcs, x_start=x_start, sigma=sigma),
    ...
    ppo_model = g_utils.train_load_model_imit(
        policy_path=f"Environments/Learning_Rate/Policies/ppo_policy_cm_imit",
        dimension=dimension,
        instance=instance,
        split=split,
        p_class=p_class,
    ...
    g_utils.save_results(results=results, policy=f"ppo_policy_cm_imit_{dimension}D_{instance}I_{p_class}C")
