...
def run(
    ...
    print(
        "---------------Running iterative imitation learning for learning-rate (cmu) adaptation---------------"
    )
    ...
    with open(
        f"Environments/Learning_Rate/Policies/ppo_policy_cm_imit_iter_{dimension}D_{instance}I_{p_class}C.pkl",
        "wb",
    ) as f:
    ...
    g_utils.save_results(
        results=results,
        policy=f"ppo_policy_cm_imit_iter_{dimension}D_{instance}I_{p_class}C",
    )
