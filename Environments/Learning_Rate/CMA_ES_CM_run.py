...
def run(
    ...
    print(
        "---------------Running learning for learning-rate (cmu) adaptation---------------"
    )
    ...
    ppo_model = g_utils.train_load_model(
        policy_path="Environments/Learning_Rate/Policies/ppo_policy_cm",
        dimension=dimension,
        instance=instance,
        split=split,
        p_class=p_class,
    ...
    g_utils.save_results(results=results, policy=f"ppo_policy_cm_{dimension}D_{instance}I_{p_class}C")
