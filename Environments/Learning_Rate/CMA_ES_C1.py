...
def run_CMAES_C1(objective_fct, x_start, sigma, h=40, f_limit=4.6*np.power(10, 18)):
...
def collect_expert_samples(
    ...
    if os.path.isfile(
        f"Environments/Learning_Rate/Samples/CMA_ES_C1_Samples_{dimension}D_{instance}I_{p_class}C.npz"
    ):
        data = np.load(
            f"Environments/Learning_Rate/Samples/CMA_ES_C1_Samples_{dimension}D_{instance}I_{p_class}C.npz"
        )
