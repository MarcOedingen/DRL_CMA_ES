...
def run(
    ...
    ppo_model = g_utils.train_load_model_imit(
        policy_path=f"Environments/h_Sigma/Policies/ppo_policy_hs_imit",
        dimension=dimension,
        instance=instance,
        split=split,
        p_class=p_class,
    ...
    g_utils.save_results(results=results, policy=f"ppo_policy_hs_imit_{dimension}D_{instance}I_{p_class}C")
